...
export type CategorieProps = {
  ...
  watch: (field: keyof WorkFilterParams) => string;
  setValue: (field: keyof WorkFilterParams, value: string) => void;
};
  ...
  return (
    <div className="md:hidden w-full overflow-x-auto bg-white py-3 px-4 shadow-sm gap-4 flex flex-col justify-between items-center">
      <form
        ...
        className="w-full"
      >
        <fieldset className="w-full flex items-center gap-4 border border-gray-300 rounded-full px-4 py-2">
          <input
            type="text"
            id="title"
          ...
          <button
            type="submit"
            className="bg-purple-700 text-white p-2 rounded-full cursor-pointer hover:bg-purple-800 transition-all duration-100"
          >
